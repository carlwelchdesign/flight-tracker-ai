...
In-progress verification evidence: [`provider-evaluation/`](../provider-evaluation/README.md) contains the common questionnaire, 66-row two-provider response matrix, outreach requests, evidence register, paired trial protocol, scorecard, five-behavior cost model, RACI, decision gates, and privacy controls; `python3 scripts/validate_ft301_evidence.py` verifies the required package, exact R-01–R-21 and S-01–S-12 response coverage, answer/review compatibility, evidence register schema/statuses, trial metrics, and cost scenarios. Its regression suite passes, while `--require-complete` intentionally fails until all evidence and question responses are in terminal reviewed states and trial/price records are complete. Public provider claims were revalidated on 2026-07-21 in [`PROVIDER_FEASIBILITY.md`](../PROVIDER_FEASIBILITY.md). Acceptance evidence remains pending; no provider has been selected.
