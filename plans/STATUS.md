...
- Next action: Sign in to the Render dashboard tab, then provision the Virginia
  staging and production services with separate Neon branch URLs and assertion
  secrets. Connect the resulting API origins to the matching Vercel
  environments, enroll the reviewer in the production Clerk organization, and
  run the private hosted smoke before promotion.
...
- Vercel Marketplace resources `neon-bronze-curtain` and `clerk-celeste-door` are available and connected to `flight-tracker-ai`. The expected encrypted Neon and Clerk variable names exist in Development, Preview, and Production; Neon pooled/direct TLS connectivity targets AWS `us-east-1`, and PostGIS `3.5.0` is enabled. Clerk now uses production keys for Production and test keys for Preview, its production domain is `flight-tracker-ai-one.vercel.app`, Organizations with required membership are enabled, and organization `Flight Tracker Portfolio` exists. Render, reviewer enrollment, Neon restore evidence, cross-service secrets, and hosted smoke remain pending.
...
- The first public-alias observation showed the original deployment still defaulting to development auth and revealed that noninteractive Vercel input had stored the five non-secret runtime settings as `[SENSITIVE]` in Production and Preview. The settings are corrected and verified. The public alias now points to production deployment `dpl_2hfw56Se2W9oSDx7fCQ4F3hHd2cb`: `/` returns HTTP 200 with the bounded `Sign in to continue` state, `/sign-in` renders Clerk's production email/password flow, the original `DEV_AUTH_SUBJECT` configuration error is absent, and CSP, referrer-policy, and content-type hardening headers are present. Operational/backend routes remain protected.
- Render's current official specification confirms managed preview environments require Pro and omit `sync: false` secrets. The zero-base-cost Blueprint defines separate free staging and production services: staging follows passing feature-branch checks during FT-404 verification, production requires manual promotion, and each receives an independent Neon branch URL and internal assertion secret. Live Render planning additionally proved free services reject `maxShutdownDelaySeconds`; the Blueprint now uses the platform default and will switch both services to `main` in the final promotion commit.
