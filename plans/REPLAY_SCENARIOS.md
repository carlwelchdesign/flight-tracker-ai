...
The milestone fixture is [m1-operations-v1.json](../fixtures/replay/m1-operations-v1.json). It includes a versioned route for the hazard-adjacent flight so the FT-204 persistence and alert lifecycle can be exercised through the normal development replay. Copy it when authoring a scenario, replace every stable identity, keep events sorted by `(offset_ms, sequence)`, and run `cargo test --workspace` to validate the file and normalization.
