...
- Raw-provider-payload deletion uses versioned tenant/provider policy, separate policy and run approvals, fixed preview counts, locked recount, a 10,000-record safety bound, completion audit, and restore-suppression tombstones. Normalized and other lifecycle classes remain open.
...
| T-08 | Data survives revocation, termination, deletion request, backup expiry, or provider deadline. | No production commercial ingestion exists. Raw payloads have two-person policy/run approval, preview/recount, deletion evidence, and restore-suppression tombstones. | Extend enforcement to normalized facts, alert/audit/session/identity classes; add scheduling/contract overrides; complete managed backup/tombstone restore drills. | F401-002, F401-008 |
