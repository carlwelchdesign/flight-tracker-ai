...
- Raw-provider-payload, authorization-audit, expired session-revocation, exclusive inactive identity, and terminal alert-history retention use versioned tenant/provider policy, separate policy and run approvals, fixed preview counts, locked recount, a 10,000-record safety bound, completion audit, and restore-suppression tombstones. Normalized facts, shared identities, and scheduling remain open.
...
| T-08 | Data survives revocation, termination, deletion request, backup expiry, or provider deadline. | No production commercial ingestion exists. Raw payload, authorization audit, expired session revocation, exclusive inactive identity, and terminal alert-history classes have two-person policy/run approval, preview/recount, deletion evidence, and restore-suppression tombstones. | Extend enforcement to normalized facts; decide shared-identity disposition; add scheduling/contract overrides; complete managed backup/tombstone restore drills. | F401-002, F401-008 |
