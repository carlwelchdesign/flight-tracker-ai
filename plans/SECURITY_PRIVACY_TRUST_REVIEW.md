...
- Raw-provider-payload, normalized operational-fact, authorization-audit, expired session-revocation, exclusive inactive identity, and terminal alert-history retention use versioned tenant/provider policy, separate policy/run or standing schedule approval, fixed preview counts plus an exact-key SHA-256 fingerprint, repeatable-read execution, locked recount, a 10,000-record safety bound, completion audit, supervised scheduled execution, durable failure attempts, restore-suppression tombstones, and administrator-only tenant integrity diagnostics. Shared identities, FT-301 contract values, managed execution, and the managed restore drill remain open.
