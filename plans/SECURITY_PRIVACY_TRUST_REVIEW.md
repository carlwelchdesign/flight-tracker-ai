...
- External aircraft positions: FT-302 implements an eligible ADSB.lol position
  layer as disabled-by-default, bounded, ephemeral, and ODbL-attributed. Replay
  remains the guaranteed path; FT-404 must revalidate the source before public
  activation.
...
| T-04 | Malformed, oversized, duplicated, out-of-order, or adversarial provider messages exhaust workers or poison state. | NOAA timeouts/retry/cadence; ADSB.lol one-megabyte response cap, timeout, bounded retry/cadence, canonical validation, freshest-record dedupe, and isolated in-memory projection; bounded channels; deterministic rule boundaries. | FT-402 must drill poison-message and backlog recovery in the hosted shape. | F401-006 |
...
| T-07 | External flight data is displayed or retained beyond the source's terms. | ADSB.lol is disabled by default and limited to a regional, server-side, ODbL-attributed, no-store position layer with zero persistent retention or export/AI path; replay remains the default. | F401-003 is closed for repository scope. FT-404 must revalidate official terms, headers, attribution, and contact guidance before public enablement. | F401-003 |
...
FT-302 maps ADSB.lol's official terms to configuration constraints, field
suppression, UI attribution, caching and zero-retention rules, rate limits,
acceptable-use controls, and automated tests. FT-404 must revalidate that
contract before a hosted environment enables the source. Replay-only remains a
valid deployment outcome. Commercial procurement remains an optional future
track and is not an FT-401 gate.
